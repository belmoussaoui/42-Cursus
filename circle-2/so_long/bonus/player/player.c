...
/*   Updated: 2022/03/21 16:49:16 by bel-mous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_player(t_render *render, t_backstage *backstage, t_player *player)
{
	...
	if (player->anim_count >= 0)
	{
		...
		if (player->anim_count < 6)
			...
			count = 1;
		blt_src(&backstage->player_move, &player->sprite,
			TILE_SIZE * count, TILE_SIZE * player->direction);
	}
	else
	{
		count = (render->frames % 4000) / 1000;
		blt_src(&backstage->player_idle, &player->sprite,
			TILE_SIZE * count, TILE_SIZE * player->direction);
	}
